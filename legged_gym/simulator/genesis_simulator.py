...
class GenesisSimulator(Simulator):
    ...
    def post_physics_step(self):
        ...
        self._key_body_pos[:] = self._robot.get_links_pos()[
            :, self._key_body_indices, :
        ]
        # Link contact state
    ...
    def reset_idx(self, env_ids):
        ...
        self._last_feet_vel[env_ids] = 0.0
        self._last_base_lin_vel[env_ids] = 0.0
        self._last_base_ang_vel[env_ids] = 0.0
    ...
    def reset_root_states(
        self, env_ids, base_pos, base_quat, base_lin_vel_w, base_ang_vel_w
    ):
        # base pos
        ...
        base_vel = torch.concat([base_lin_vel_w, base_ang_vel_w], dim=1)
        self._robot.set_dofs_velocity(
            velocity=base_vel, dofs_idx_local=[0, 1, 2, 3, 4, 5], envs_idx=env_ids
        )
        ...
        self._base_ang_vel[env_ids] = quat_rotate_inverse(
            self._base_quat[env_ids], self._robot.get_ang()[env_ids]
        )
    ...
    def push_robots(self):
        ...
        dofs_vel = (
            self._robot.get_dofs_velocity()
        )  # (num_envs, num_dof) [0:3] ~ base_link_vel
        push_vel = torch_rand_float(
            -max_push_vel_xy, max_push_vel_xy, (self._num_envs, 2), self._device
        )
    ...
    def push_links(self):
        ...
        push_force = (
            torch.rand((self._num_envs, self._robot.n_links, 3), device=self._device)
            * 2
            * max_force
            - max_force
        )
        ...
        self._scene.sim.rigid_solver.apply_links_external_force(
            push_force, links_idx=all_link_idx
        )
    ...
    def _parse_cfg(self):
        ...
        self._batch_dofs_links_info = (
            self._cfg.domain_rand.randomize_joint_armature
            or self._cfg.domain_rand.randomize_joint_friction
            or self._cfg.domain_rand.randomize_joint_damping
        )
        if self._cfg.sensor.add_depth:
            self.frame_count = 0
    ...
    def _create_sim(self):
        ...
        self._scene = gs.Scene(
            sim_options=gs.options.SimOptions(
                dt=self._sim_params["dt"], substeps=self._sim_params["substeps"]
            ),
        ...
        if mesh_type == "plane":
            ...
            raise NotImplementedError(
                "Trimesh terrain is not validated yet in Genesis, please use heightfield for now."
            )
            self._terrain = Terrain(self._cfg.terrain)
        ...
        if self._cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            self._terrain_x_range[1] = (
                self._cfg.terrain.border_size
                + self._cfg.terrain.num_rows * self._cfg.terrain.terrain_length
                - 1.0
            )
            self._terrain_y_range[0] = -self._cfg.terrain.border_size + 1.0
            self._terrain_y_range[1] = (
                self._cfg.terrain.border_size
                + self._cfg.terrain.num_cols * self._cfg.terrain.terrain_width
                - 1.0
            )
    ...
    def _create_envs(self):
        ...
        # name to indices
        self._dof_indices = [
            self._robot.get_joint(name).dof_start for name in self._cfg.asset.dof_names
        ]
        ...
        self._feet_names = [
            link.name
            for link in self._robot.links
            if self._cfg.asset.foot_name in link.name
        ]
        self._feet_indices = find_link_indices(self._feet_names)
        print(
            f"feet names: {self._feet_names}, feet link indices: {self._feet_indices}"
        )
        ...
        if hasattr(self._cfg.asset, "dof_vel_limits"):
            self._dof_vel_limits = torch.tensor(
                self._cfg.asset.dof_vel_limits, device=self._device
            ).unsqueeze(0)
        self._torque_limits = self._robot.get_dofs_force_range(self._dof_indices)[1]
    ...
    def _init_buffers(self):
        ...
        self._global_gravity = torch.tensor(
            [0.0, 0.0, -1.0], device=self._device, dtype=torch.float
        ...
        self._base_quat_gs = torch.zeros(
            (self._num_envs, 4), device=self._device, dtype=torch.float
        )  # quaternion in genesis definition, wxyz
        self._base_euler = torch.zeros(
            (self._num_envs, 3), device=self._device, dtype=torch.float
        )
        self._link_contact_forces = torch.zeros(
            (self._num_envs, self._robot.n_links, 3),
            device=self._device,
            dtype=torch.float,
        )
        ...
        self._key_body_pos = torch.zeros(
            (self._num_envs, len(self._key_body_indices), 3),
            device=self._device,
            dtype=torch.float,
        )
    ...
    def _get_env_origins(self):
        ...
        if self._cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            self._terrain_origins = (
                torch.from_numpy(self._terrain.env_origins)
                .to(self._device)
                .to(torch.float)
            )
            self._env_origins[:] = self._terrain_origins[
                self._terrain_levels, self._terrain_types
            ]
        else:
            ...
            if (
                num_rows * self._cfg.env.env_spacing
                > self._cfg.terrain.plane_length / 2
                or num_cols * self._cfg.env.env_spacing
                > self._cfg.terrain.plane_length / 2
            ):
                spacing = min(
                    (self._cfg.terrain.plane_length / 2) / (num_rows - 1),
    ...
    def _update_surrounding_heights(self):
        ...
        points = quat_apply_yaw(
            self._base_quat.repeat(1, self._num_height_points), self._height_points
        ...
        self._measured_heights = (
            heights.view(self._num_envs, -1) * self._cfg.terrain.vertical_scale
        )

    def _calc_terrain_info_around_feet(self):
        """Finds neighboring points around each foot for terrain height measurement."""
        if self._cfg.terrain.mesh_type == "plane":
            # For a flat plane, heights are 0 and normal vectors are strictly pointing up (-1 in Genesis Z-up)
            for i in range(len(self._feet_indices)):
                self._normal_vector_around_feet[:, i * 3 : i * 3 + 3] = torch.tensor(
                    [0.0, 0.0, -1.0], device=self._device
                )
        ...
        dx = ((heights2 - heights1) / (self._cfg.terrain.horizontal_scale * 2)).view(
            self._num_envs, -1
        )
        dy = ((heights4 - heights3) / (self._cfg.terrain.horizontal_scale * 2)).view(
            self._num_envs, -1
        )
        for i in range(len(self._feet_indices)):
            normal_vector = torch.cat(
                (
                    dx[:, i].unsqueeze(1),
                    dy[:, i].unsqueeze(1),
                    -1 * torch.ones_like(dx[:, i].unsqueeze(1)),
                ),
                dim=-1,
            ).to(self._device)
        ...
        for i in range(9):
            self._height_around_feet[:, :, i] = (
                eval(f"heights{i + 1}").view(self._num_envs, -1)[:]
    ...
    def _randomize_friction(self, env_ids=None):
        """Randomize friction of all links AND the terrain"""
        min_friction, max_friction = self._cfg.domain_rand.friction_range

        # gs.rand tạo ra giá trị từ 0 đến 1
        ratios_rand = gs.rand((len(env_ids), 1), dtype=float).to(self._device)
        
        min_f = torch.full((len(env_ids), 1), min_friction, device=self._device, dtype=torch.float32)
        max_f = torch.full((len(env_ids), 1), max_friction, device=self._device, dtype=torch.float32)

        # Tính toán ma sát tương ứng từng loại địa hình (nếu được cấu hình)
        if hasattr(self._cfg.domain_rand, "terrain_friction_ranges") and hasattr(self, "_terrain_types"):
            choices = self._terrain_types[env_ids].float().unsqueeze(1) / self._cfg.terrain.num_cols
            # expected dict: {0.6: [0.05, 1.5], 1.0: [0.5, 1.5]}
            for limit, f_range in self._cfg.domain_rand.terrain_friction_ranges.items():
                mask = choices < limit
                min_f[mask] = f_range[0]
                max_f[mask] = f_range[1]
                choices[mask] = 999.0  # Ngăn các mốc cao hơn ghi đè mốc đã check

        ratios = ratios_rand * (max_f - min_f) + min_f
        ratios = ratios.repeat(1, self._robot.n_links)

        # Lưu lại giá trị ma sát vào buffer để sau này in debug hoặc cho mạng Teacher nhìn
        self._friction_values[env_ids] = ratios[:, 0].unsqueeze(1).detach().clone()

        # 2. Cấu hình ma sát cho ROBOT (Bàn chân và các link)
        self._robot.set_friction_ratio(
            ratios, torch.arange(0, self._robot.n_links), env_ids
        )

        # 3. Cấu hình ma sát cho MẶT SÀN (Terrain) - QUAN TRỌNG NHẤT
        # Chúng ta lấy giá trị ma sát của env đó (ratios[:, 0:1]) áp dụng cho sàn của chính env đó
        if hasattr(self, "_gs_terrain"):
            self._gs_terrain.set_friction_ratio(ratios[:, 0:1], envs_idx=env_ids)
    ...
    def _randomize_com_displacement(self, env_ids):
        ...
        com_displacement = torch.zeros(
            (len(env_ids), 1, 3), dtype=torch.float, device=self._device
        )

        com_displacement[:, 0, 0] = (
            gs.rand((len(env_ids), 1), dtype=float).squeeze(1)
            * (max_displacement_x - min_displacement_x)
            + min_displacement_x
        )
        com_displacement[:, 0, 1] = (
            gs.rand((len(env_ids), 1), dtype=float).squeeze(1)
            * (max_displacement_y - min_displacement_y)
            + min_displacement_y
        )
        com_displacement[:, 0, 2] = (
            gs.rand((len(env_ids), 1), dtype=float).squeeze(1)
            * (max_displacement_z - min_displacement_z)
            + min_displacement_z
        )
    ...
    def _randomize_joint_armature(self, env_ids):
        ...
        armature = (
            torch.rand((len(env_ids),), dtype=torch.float, device=self._device)
            * (max_armature - min_armature)
            + min_armature
        )
        ...
        self._robot.set_dofs_armature(armature, self._dof_indices, envs_idx=env_ids)
    ...
    def _randomize_joint_friction(self, env_ids):
        ...
        self._robot.set_dofs_frictionloss(friction, self._dof_indices, envs_idx=env_ids)
    ...
    def _update_depth_images(self):
        ...
        self.depth_images = (self.depth_images - near_clip) / (
            far_clip - near_clip
        ) - 0.5
    ...
    def _create_trimesh(self):
        ...
        trimesh_terrain_path = os.path.join(
            LEGGED_GYM_ROOT_DIR, "resources", "terrains", "trimesh_terrain.stl"
        )
    ...
    def _setup_depth_camera(self):
        ...
        self.depth_camera = self._scene.add_sensor(
            gs.sensors.DepthCamera(pattern=depth_pattern, **sensor_kwargs)
        )
    ...
    def _randomize_restitution(self):
        pass
