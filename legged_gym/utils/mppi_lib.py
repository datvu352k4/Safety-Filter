...
class MPPI(nn.Module):
    ...
    def __init__(
        ...
        self._device = device if torch.cuda.is_available() and device.type == "cuda" else torch.device("cpu")
        self._dtype = dtype
    ...
    def forward(
        ...
        # Step 1: Sample action sequences with Temporal Correlation (AR-1 noise)
        # Generate noise samples using reparametrization trick
        self._action_noises = self._noise_distribution.rsample(
            sample_shape=self._sample_shape
        ) # [K, H, dim_control]

        # Apply temporal smoothing to noise: noise_t = beta * noise_{t-1} + (1-beta) * current_noise
        beta = 0.85 # Tăng độ mượt lên mức cao để khử rung
        for t in range(1, self._horizon):
            self._action_noises[:, t, :] = beta * self._action_noises[:, t-1, :] + (1 - beta) * self._action_noises[:, t, :]
        ...
        # Store and SHIFT for warm-starting next iteration (CRITICAL for smoothness)
        self._previous_action_seq = torch.roll(optimal_action_seq, shifts=-1, dims=0)
        self._previous_action_seq[-1] = 0.0 # Reset last step to zero or default velocity
