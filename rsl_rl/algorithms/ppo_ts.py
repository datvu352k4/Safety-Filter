...
class PPO_TS(PPO):
    ...
    def __init__(self,
                 ...
                 use_aux_terrain_loss=False,  # Ignored in baseline
                 ):
        ...
        encoder_params = list(self.actor_critic.history_encoder.parameters())
        self.history_encoder_optimizer = optim.Adam(encoder_params, lr=encoder_lr)
    ...
    def _compute_rl_loss(self, obs_batch, privileged_obs_batch, critic_obs_batch, 
        ...
        # Auxiliary friction disentanglement loss on privilege_encoder:
        #   z[0] → friction (privileged_obs dim 0)
        #   Scale 0.05: guides z0 to encode friction clearly without overriding RL.
        #   NOTE: roughness aux (z1) removed — height_var_ema≈0 for both terrains at high speed,
        #   making target=-1 constant for all cases → z1 saturation. Let RL + imitation learn it.
        z_priv = self.actor_critic.privilege_encoder(privileged_obs_batch)
        friction_norm = (privileged_obs_batch[:, 0:1].clamp(0.05, 1.7) - 0.05) / 1.65  # [0,1]
        aux_loss = F.mse_loss(z_priv[:, 0:1], friction_norm * 2.0 - 1.0) * 0.05  # friction → z0 ∈ [-1,1]
        loss = loss + aux_loss
    ...
    def _compute_encoder_loss(self, obs_histories_batch, privileged_obs_batch, terminated_batch):
        ...
        z_t = self.actor_critic.history_encoder(obs_histories_batch)
        
        with torch.no_grad():
            encoder_targets = self.actor_critic.privilege_encoder(privileged_obs_batch)

        # Standard imitation loss (student → teacher)
        imitation_loss = F.mse_loss(z_t * terminated_batch, encoder_targets * terminated_batch)

        # Friction auxiliary on student: same as teacher, ensures z_student[0] encodes friction
        friction_norm = (privileged_obs_batch[:, 0:1].clamp(0.05, 1.7) - 0.05) / 1.65
        aux_loss = F.mse_loss(z_t[:, 0:1], friction_norm * 2.0 - 1.0) * 0.05

        return imitation_loss + aux_loss