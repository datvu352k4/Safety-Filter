...
class ActorCriticTS(nn.Module):
    ...
    def __init__(self,  
        ...
        # Privilege encoder
        if len(privilege_encoder_hidden_dims) == 0:
            assert num_privilege_encoder_input == num_latent_dims, "Input and latent dimensions must match if using Identity privilege encoder."
            self.privilege_encoder = nn.Identity()
        else:
        ...
        if history_encoder_type == "MLP":
            ...
            for l in range(len(history_encoder_hidden_dims)):
                if l == len(history_encoder_hidden_dims) - 1:
                    ...
                    history_encoder_layers.append(activation)
            history_encoder_layers.append(nn.LayerNorm(num_latent_dims))
